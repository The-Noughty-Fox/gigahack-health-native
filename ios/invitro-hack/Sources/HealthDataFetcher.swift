...
class HealthDataFetcher: DefaultInitializable, Module, EnvironmentAccessible {
    ...
    func fetchLastTwoWeeksQuantityDataHeart(
        for identifier: HKQuantityTypeIdentifier,
        unit: HKUnit,
        options: HKStatisticsOptions
        ...
        quantityCounts.enumerateStatistics(
            ...
            if let quantity = statistics.maximumQuantity(), let quantityMin = statistics.minimumQuantity(), let quantityAvarage = statistics.averageQuantity() {
                dailyData.append(HeartData(
                    max: Int(quantity.doubleValue(for: HKUnit.init(from: "count/s"))*60),
                    min: Int(quantityMin.doubleValue(for: HKUnit.init(from: "count/s"))*60),
                    avarage: Int(quantityAvarage.doubleValue(for: HKUnit.init(from: "count/s"))*60),
                    startDate: statistics.startDate,
                    endDate: statistics.endDate
                ))
            } else {
                dailyData.append(nil)
            }
        }
    ...
    /// - Throws: `HealthDataFetcherError` if the data cannot be fetched.
    func fetchLastTwoWeeksHeartRate() async throws -> [HeartData?] {
        try await fetchLastTwoWeeksQuantityDataHeart(
            for: .heartRate,
            unit: .count(),
